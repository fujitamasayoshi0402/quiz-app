...
- draw.io の原本と、書き出した SVG の組。本体は S3 に置き、DB には行だけを持つ（[ADR-0017](adr/0017-deliver-figures-with-cloudfront-signed-urls.md)）
- **一度置いたら変えない。** 描き直した図は、新しい図として置く
- テナントに属する。別テナントの図は、ID を指定しても見えない

変えないのは、配る URL のキャッシュを無効にしなくて済むようにするためです。
差し替えは「新しい図を置き、クイズが指す図を替える」操作になります。

クイズとの紐付け（1 問に付ける図の数、解説の中の位置）は、管理画面で図を作る段（DEV-69）で決めます。
