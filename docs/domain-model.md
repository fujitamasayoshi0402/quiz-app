...
未回答優先は回答履歴を参照します。**未回答が尽きたら、回答済みのクイズで残りを埋めます**（DEV-21 で確定）。
一度正解した問題を二度と出さない理由がなく、指定した出題数に満たない結果は利用者の期待と異なるためです。

出題モードとは別に、**正誤と解説をいつ返すか**という軸があります（[要件定義](requirements.md)）。
ただしこれは表示の選択であり、**ドメインの構造には現れません。**
...
### 「1 回の挑戦」をどう持つか（DEV-22 で確定）

現在の回答は 1 回答 1 行で、**どの挑戦に属するかを持ちません。**
「10 問中 7 問正解」の結果画面と、ランキングの集計には、この単位が必要です。

あわせて、同じクイズを復習で解き直したときにどの回答をスコアに採用するかも決まっていません。
回答 API の設計（DEV-22）で扱います。
