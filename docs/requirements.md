...
| 画面 | 内容 | Phase |
| --- | --- | --- |
| 出題条件の選択 | カテゴリ → 難易度（またはレベル）→ 出題モード → フィードバック方式 | 1 |
...
flowchart TD
    ...
    Play -->|カテゴリ → 難易度 → 出題モード| Mode{"フィードバック方式"}
    Mode -->|学習モード| Learn["回答"]
    Learn --> Judge["正誤と解説"]
    Judge -->|次の問題| Learn
...
上限は「これ以上は許さない」ラインであり、実際の体験を決めるのは既定値です。
1 問ずつ解く形式で 100 問は 30〜60 分かかるため、既定は 10 問とし、利用者が増やせるようにします。

### 未回答が尽きたとき

未回答優先モードで候補を出し切ったら、**回答済みのクイズで残りを埋めます。**
「もう解く問題がありません」で終わらせません。

このアプリの用途は知識の定着であり、一度正解した問題を二度と出さない理由がないためです。
出題数を指定した利用者にとっても、指定した数に満たない結果は期待と異なります。
...
回答の途中で離脱しても、**同じブラウザであれば続きから再開できます。**

出題されたクイズ ID の並びと現在位置をブラウザに保存します（localStorage）。
サーバー側に出題セッションを持たせないため、Phase 1 の実装量に収まります。

**回答そのものは 1 問ごとにサーバーへ保存されます。** ブラウザに残すのは出題順と現在位置だけです。
離脱で失われるのは「どの問題を、どの順で出していたか」であり、解いた実績は残ります。
...
いずれも Phase 1 では許容します。サーバー保存に移行する場合は「出題セッション」をドメインに追加することになりますが、
**保存先が変わるだけで画面の作りは変わらない**ため、後からの移行は可能です。
...
| 項目 | 決める時期 |
| --- | --- |
| 「1 回の挑戦」をデータとして持つか（結果画面とランキングに必要） | 回答 API（DEV-22） |
| 同じクイズを解き直したときに、どの回答をスコアに採用するか | 回答 API（DEV-22） |
| RLS ポリシーに削除条件を含めるか（含めると削除済み一覧を取得できない） | DB 設計（DEV-17） |
| 連鎖削除をアプリケーション側で行うか、DB に委ねるか | DB 設計（DEV-17） |
